...
import { PROJECT_STATUS_MAP, STAGE_STATUS_MAP, SERVICE_STATUS_MAP, TASK_STATUS_MAP, TASK_PRIORITY_MAP, MEASUREMENT_STATUS_MAP, EXPENSE_STATUS_MAP, OCCURRENCE_STATUS_MAP, OCCURRENCE_PRIORITY_MAP } from '@/lib/constants'
import type { ProjectStatus, StageStatus, ServiceStatus, TaskStatus, TaskPriority, MeasurementStatus, ExpenseStatus, OccurrenceStatus, OccurrencePriority } from '@prisma/client'

interface StatusBadgeProps {
  status: ProjectStatus | StageStatus | ServiceStatus | TaskStatus | TaskPriority | MeasurementStatus | ExpenseStatus | OccurrenceStatus | OccurrencePriority
  type: 'project' | 'stage' | 'service' | 'task' | 'priority' | 'measurement' | 'expense' | 'occurrence' | 'occurrencePriority'
  className?: string
}
...
export function StatusBadge({ status, type, className }: StatusBadgeProps) {
  ...
  const getMap = () => {
    switch (type) {
      ...
      case 'service': return SERVICE_STATUS_MAP[status as ServiceStatus]
